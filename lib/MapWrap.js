 ...
 * @param {Number} [cacheSize] - specify max size of all caches. 10 by default.
 */
class MapWrap {
  constructor(key, cacheSize=10) {
    if (!key) {
    ...
    this.reverseGeoCache = new LRUCache(cacheSize);
    this.geoCache = new LRUCache(cacheSize);
    this.directionsCache = new LRUCache(cacheSize);
    this.placesCache = new LRUCache(cacheSize);
    this.placeDetailsCache = new LRUCache(cacheSize);
  }
  ...
  async placeDetails(placeId) {
    ...
    let data = this.placeDetailsCache.get(placeId);
    if (!data) {
      try {
        const response = await this.googleMaps.get(`/place/details/json?placeid=${placeId}&key=${this.API_KEY}`);
      
        this.placeDetailsCache.set(placeId, response.data);
