...
class DirectionsWrapper {
   ...
   */
  getRoutes() {
    if (this.googleResponse && this.googleResponse.routes) {
      if (Array.isArray(this.googleResponse.routes) && this.googleResponse.routes.length > 0) {
        return this.googleResponse.routes;
      } else if (Array.isArray(this.googleResponse.routes)) {
        return [];
      }
    }
    return null;
